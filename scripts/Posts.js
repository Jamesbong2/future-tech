  ...
  return `<li class="list__item">
    <article class="blog-card container">
      <div class="blog-card__author person-card">
        <img src="${post.authorImage}" width="80" height="80" alt="${post.author}">
...
function renderPostsError(listElement, onRetry) {
  ...
  const errorElement = ErrorState({
    ...
    message: "We couldn’t load the latest posts right now. Please try again.",
    buttonText: "Retry",
    onAction: onRetry,
  });

  listItem.append(errorElement);
...
export function initPosts() {
  ...
  async function loadPosts(category, listElement) {
    try {
      ...
      console.error(error);
      const handleRetry = () => {
        loadPosts(category, listElement);
      };

      renderPostsError(listElement, handleRetry);
    }
  }

  const tabsElement = document.querySelector("[data-js-tabs-root]");

  const destroyTabs = initTabs(tabsElement, ({ category, panel }) => {
